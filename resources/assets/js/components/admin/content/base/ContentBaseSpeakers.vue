        ...
        <div class="box">
            ...
            <div class="box-body">
                <div class="col-md-12">
                    ...
                    <div class="col-md-6 form-group has-feedback"
                         :class="{ 'has-error': form.errors.has('confirmed_heading') }">
                        <label>Confirmed speakers heading</label>

                        <input type="text" class="form-control" placeholder="Confirmed speakers heading" name="confirmed_heading" value=""
                               v-model="form.confirmed_heading" autofocus/>

                        <transition name="fade">
                        <span class="help-block"
                              v-if="form.errors.has('confirmed_heading')"
                              v-text="form.errors.get('confirmed_heading')"
                              id="validation_error_confirmed_heading"></span>
                        </transition>
                    </div>

                    <div class="col-md-6 form-group has-feedback"
                         :class="{ 'has-error': form.errors.has('previous_heading') }">
                        <label>Previous speakers heading</label>

                        <input type="text" class="form-control" placeholder="Previous speakers heading" name="previous_heading" value=""
                               v-model="form.previous_heading" autofocus/>

                        <transition name="fade">
                        <span class="help-block"
                              v-if="form.errors.has('previous_heading')"
                              v-text="form.errors.get('previous_heading')"
                              id="validation_error_previous_heading"></span>
                        </transition>
                    </div>

                    <div class="col-md-6 form-group has-feedback"
                         :class="{ 'has-error': form.errors.has('confirmed_text') }">
                        <label>Confirmed speakers text (html allowed)</label>

                        <editor v-model="form.confirmed_text" @init="editorInit();" lang="html" theme="chrome" width="100%" height="80px"></editor>

                        <transition name="fade">
                        <span class="help-block"
                              v-if="form.errors.has('confirmed_text')"
                              v-text="form.errors.get('confirmed_text')"
                              id="validation_error_confirmed_text"></span>
                        </transition>
                    </div>

                    <div class="col-md-6 form-group has-feedback"
                         :class="{ 'has-error': form.errors.has('previous_text') }">
                        <label>Previous speakers text (html allowed)</label>

                        <editor v-model="form.previous_text" @init="editorInit();" lang="html" theme="chrome" width="100%" height="80px"></editor>

                        <transition name="fade">
                        <span class="help-block"
                              v-if="form.errors.has('previous_text')"
                              v-text="form.errors.get('previous_text')"
                              id="validation_error_previous_text"></span>
                        </transition>
                    </div>
    ...
    export default {
        ...
        data: function () {
            let form = new Form({
                ...
                confirmed_heading: this.valueOrEmpty(this.content.confirmed_heading),
                previous_heading: this.valueOrEmpty(this.content.previous_heading),
                confirmed_text: this.valueOrEmpty(this.content.confirmed_text),
                previous_text: this.valueOrEmpty(this.content.previous_text),
            })
        ...
        methods: {
            ...
            submit () {
                this.busy = true;
                this.form.post('/a/content/base/speakers')
                    .then(response => {
                        var component = this;
                        this.busy = false;
