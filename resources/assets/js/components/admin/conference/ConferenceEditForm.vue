<template>
    <form method="post" @submit.prevent="submit" @keydown="clearErrors($event.target.name)" @change="clearErrors($event.target.name)">
        <div class="alert alert-success text-center" v-show="busy" id="result"> Editing conference &nbsp;&nbsp;
        ...
        <div class="row">
            <div class="col-xs-12">
                <button type="submit" class="btn btn-primary btn-block btn-flat" :disabled="busy">Submit
                </button>
            </div>
        </div>
    </form>
...
<script>
  ...
  export default {
    ...
    computed: {
      form: function () {
        return new Form({
          id: this.conferenceData.id,
          name: this.conferenceData.name,
          url_slug: this.conferenceData.url_slug,
          frontend_theme: this.conferenceData.frontend_theme,
          admin_theme: this.conferenceData.admin_theme,
        })
      }
    },
    watch: {
    ...
    methods: {
      submit () {
        ...
        this.busy = true;
        this.form.post('/a/conference/edit' + this.conference.url_slug)
          .then(response => {
            var component = this;
            this.busy = false;
